...
DEFINE
	...
	VAR __DM3FilterTable = 
		TREATAS({"Renting"}, 'Creditos'[Tipo de Producto])

	VAR __ValueFilterDM9 = 
		FILTER(
			KEEPFILTERS(
				SUMMARIZECOLUMNS(
	...
	VAR __DS0Core = 
		SUMMARIZECOLUMNS(
			ROLLUPADDISSUBTOTAL(
				'Creditos'[Tipo de Producto], "IsGrandTotalRowTotal",
				'Empresas'[Compañia Alias], "IsDM1Total", NONVISUAL(__DM3FilterTable)
			),
			__DS0FilterTable,
			__DS0FilterTable2,
			__DS0FilterTable3,
...
ORDER BY
	...
	'Creditos'[Tipo de Producto] DESC,
	[IsDM1Total] DESC,
	'Empresas'[Compañia Alias]
